<properties 
    pageTitle="Cargar archivos a una cuenta de Media Services con REST | Microsoft Azure" 
...
# <a name="upload-files-into-a-media-services-account-using-rest"></a>Cargar archivos a una cuenta de Media Services con REST
...
El flujo de trabajo básico para cargar activos se divide en las secciones siguientes:

- Crear un activo
- Cifrar un activo (opcional)
- Cargar un archivo al almacenamiento de blobs

AMS también le permite cargar activos de forma masiva. Para obtener más información, consulte [esta](media-services-rest-upload-files.md#upload_in_bulk) sección.

##<a name="upload-assets"></a>Cargar activos
...
Una de las propiedades que puede especificar al crear un activo es **Opciones**. **Options** es un valor de enumeración que describe las opciones de cifrado que pueden crearse con un activo. Un valor válido es uno de los valores de la lista siguiente, no una combinación de valores. 

- **Ninguno** = **0**: no se usará cifrado. Este es el valor predeterminado. Tenga en cuenta que al usar esta opción su contenido no está protegido al transmitirlos o al resto de almacenamiento.
    Si va a ofrecer un MP4 mediante descarga progresiva, use esta opción. 

- **StorageEncrypted** = **1**: especifique si desea que los archivos se cifrada con cifrado AES-256 para cargar y el almacenamiento de.

    Si el activo es almacenamiento cifrado, debe configurar Directiva de entrega de activos. Para obtener más información, vea [Directiva de entrega de activos de configuración](media-services-rest-configure-asset-delivery-policy.md).
...
- **EnvelopeEncryptionProtected** = **4**: especifique si va a cargar HLS cifrados con archivos AES. Tenga en cuenta que deben ha codificado y cifrado administrador transformar los archivos.

>[AZURE.NOTE]Si su activo usará el cifrado, debe crear un **ContentKey** y vincularlo a su activo, como se describe en el tema siguiente:[cómo crear un ContentKey](media-services-rest-create-contentkey.md). Tenga en cuenta que después de cargar los archivos en el activo, deberá actualizar las propiedades de cifrado de la entidad **AssetFile** con los valores que obtuvo durante el cifrado de **activos** . Realizar con la solicitud HTTP **Combinar** . 
...
Después de cargar el archivo de medios digitales en un contenedor de blob, usará la solicitud HTTP **Combinar** para actualizar la AssetFile con información sobre el archivo multimedia (como se muestra más adelante en el tema). 
...
Antes de cargar los archivos en el almacenamiento de blobs, establezca el acceso derechos de directiva para escribir en un activo. Para ello, publique una solicitud HTTP al conjunto de entidades AccessPolicies. Definir un valor de DurationInMinutes tras la creación o recibirá un mensaje de error de servidor interno 500 en respuesta. Para obtener más información sobre AccessPolicies, consulte [AccessPolicy](http://msdn.microsoft.com/library/azure/hh974297.aspx).
...
Para recibir la dirección URL de carga real, cree un localizador SA. Localizadores definen la hora de inicio y el tipo de extremo de conexión para los clientes que deseen tener acceso a archivos de un activo. Puede crear varias entidades de localizador para AccessPolicy activos par y gestionar solicitudes de otro cliente y necesidades. Cada uno de estos localizadores use el valor de hora de inicio más el valor de DurationInMinutes de la AccessPolicy para determinar la longitud de tiempo que se puede utilizar una dirección URL. Para obtener más información, consulte [localizador](http://msdn.microsoft.com/library/azure/hh974308.aspx).
...
##<a id="upload_in_bulk"></a>Cargar activos de forma masiva

###<a name="create-the-ingestmanifest"></a>Crear la IngestManifest

El IngestManifest es un contenedor para un conjunto de activos, archivos de activos e información de estadística que se pueden usar para determinar el progreso de masa recopila para el conjunto.
...
###<a name="create-assets"></a>Crear activos

Antes de crear el IngestManifestAsset, debe crear el activo que se completará con recopila de forma masiva. Un activo es un contenedor de varios tipos de o conjuntos de objetos en servicios de medios, incluyendo vídeo, audio, imágenes, colecciones de miniaturas, pistas de texto y archivos de subtítulos. En la API de REST, crear un activo debe enviar una solicitud de HTTP POST a Microsoft Azure Media Services y colocar cualquier información de propiedad sobre su activo en el cuerpo de la solicitud. En este ejemplo, se crea el activo mediante la opción StorageEncrption(1) incluida en el cuerpo de la solicitud.
...
###<a name="create-the-ingestmanifestassets"></a>Crear la IngestManifestAssets

IngestManifestAssets representan activos dentro de un IngestManifest que se usan con recopila de forma masiva. El vínculo básicamente activo al manifiesto. Azure Media Services internamente inspecciona la carga de archivos en función de la colección de IngestManifestFiles asociada a la IngestManifestAsset. Una vez que se cargan estos archivos, el activo se ha completado. Puede crear un nuevo IngestManifestAsset con una solicitud de HTTP POST. En el cuerpo de la solicitud, incluya el identificador de IngestManifest y el Id. de activo debe vincular la IngestManifestAsset de masa recopila.
...
###<a name="create-the-ingestmanifestfiles-for-each-asset"></a>Crear la IngestManifestFiles para cada activo

Un IngestManifestFile representa un objeto blob real de audio o vídeo que se cargarán como parte del bloque recopila de un activo. Cifrado relacionados con propiedades no son necesarias a menos que el activo está utilizando una opción de cifrado. En el ejemplo se utilizan en esta sección muestra la creación de un IngestManifestFile que utiliza StorageEncryption para el activo creado previamente.
...
###<a name="upload-the-files-to-blob-storage"></a>Cargue los archivos al almacenamiento de blobs

Puede usar cualquier aplicación de cliente de alta velocidad capaz de cargar los archivos de activos en el contenedor de almacenamiento de blobs Uri proporcionada por la propiedad BlobStorageUriForUpload de la IngestManifest. Uno de los servicios de carga de alta velocidad notables es [Aspera On Demand de aplicación de Azure](http://go.microsoft.com/fwlink/?LinkId=272001).

###<a name="monitor-bulk-ingest-progress"></a>Monitor masa recopilar progreso

Puede supervisar el progreso de masa recopila operaciones para un IngestManifest mediante el sondeo de la propiedad de estadísticas de la IngestManifest. Propiedad es un tipo complejo, [IngestManifestStatistics](https://msdn.microsoft.com/library/azure/jj853027.aspx). Para sondear la propiedad estadísticas, enviar una solicitud HTTP GET pasando el Id. IngestManifest
 

##<a name="create-contentkeys-used-for-encryption"></a>Crear ContentKeys utilizado para el cifrado

Si su activo usará el cifrado, debe crear el ContentKey que se utilizará para el cifrado antes de crear los archivos de activos. Para el cifrado de almacenamiento, las siguientes propiedades deben incluirse en el cuerpo de la solicitud.
...
### <a name="link-the-contentkey-to-the-asset"></a>Vincular la ContentKey al activo

La ContentKey está asociado a uno o más activos al enviar una solicitud de HTTP POST. La solicitud siguiente es un ejemplo al que vincular el ejemplo ContentKey activo ejemplo por Id.
